use sntl_core::query::{DeleteQuery, InsertQuery};
...
fn delete_build_still_works() {
    let q = DeleteQuery::new("users").where_id(sntl_core::types::Value::Int(1));
    let (sql, binds) = q.build();
...
async fn execute_api_compiles(conn: &mut sentinel_driver::Connection) {
    ...
    let q2 = DeleteQuery::new("users").where_id(sntl_core::types::Value::Int(1));
